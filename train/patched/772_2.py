...
class Hashing():
    ...
    def Standard_Multi_Hash(self,Data:str):
        ...
        c = (b[0].split("G") if "G" in b[0] else (b[0].split("g") if "g" in b[0] else (b[0].split("v") if "v" in b[0] else b[0].split("x")))); d=[]; e=[]
        ...
        for i in range(len(c)):
            ...
            e.append(''.join(d))
            final_hash = self.BLAKE2(bytes(str(e[0]).encode()))
            return(final_hash)
    ...
    def SHA256(self,data:str):
        ...
        return self.__Salt(hash_digest,salt=self.salt)
