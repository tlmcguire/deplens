...
def test_solid(tmp_path):
    f = "solid.7z"
    with open(testdata_path.joinpath(f), mode="rb") as fp:
        archive = py7zr.SevenZipFile(fp)
...
@pytest.mark.files
def test_empty():
    with open(testdata_path.joinpath("empty.7z"), mode="rb") as fp:
        archive = py7zr.SevenZipFile(fp)
...
def test_github_14(tmp_path):
    with open(testdata_path.joinpath("github_14.7z"), mode="rb") as fp:
         archive = py7zr.SevenZipFile(fp)
...
def _test_umlaut_archive(filename: str, target: pathlib.Path, return_dict: bool):
    with open(testdata_path.joinpath(filename), mode="rb") as fp:
         archive = py7zr.SevenZipFile(fp)
         if not return_dict:
...
def test_bugzilla_4(tmp_path):
    with open(testdata_path.joinpath("bugzilla_4.7z"), mode="rb") as fp:
        archive = py7zr.SevenZipFile(fp)
        expected = [
            {
...
def test_extract_symlink(tmp_path):
    with open(testdata_path.joinpath("symlink.7z"), mode="rb") as fp:
        archive = py7zr.SevenZipFile(fp)
...
def test_lzma2bcj(tmp_path):
    ...
    with open(testdata_path.joinpath("lzma2bcj.7z"), mode="rb") as fp:
        archive = py7zr.SevenZipFile(fp)
        assert archive.getnames() == [
            "mingw64",
            "mingw64/bin",
...
def test_lzma2bcj2(tmp_path):
    ...
    with pytest.raises(UnsupportedCompressionMethodError):
        with open(testdata_path.joinpath("lzma2bcj2.7z"), mode="rb") as target:
...
def test_lzma2bcj2_2(tmp_path):
    ...
    with pytest.raises(UnsupportedCompressionMethodError):
        with open(testdata_path.joinpath("lzma2bcj2_2.7z"), mode="rb") as target:
            archive = py7zr.SevenZipFile(target)
            archive.extractall(path=tmp_path)
            archive.close()
...
def test_extract_lzma_1(tmp_path):
    with open(testdata_path.joinpath("lzma_1.7z"), mode="rb") as target:
...
def test_extract_lzma2_1(tmp_path):
    with open(testdata_path.joinpath("lzma2_1.7z"), mode="rb") as target:
        with py7zr.SevenZipFile(target) as ar:
            _dict = ar.readall()
...
def test_zerosize(tmp_path):
    with open(testdata_path.joinpath("zerosize.7z"), mode="rb") as target:
        archive = py7zr.SevenZipFile(target)
        archive.extractall(path=tmp_path)
        archive.close()
...
@pytest.mark.files
def test_skip():
    with open(testdata_path.joinpath("test_1.7z"), mode="rb") as fp:
        archive = py7zr.SevenZipFile(fp)
        for i, cf in enumerate(archive.files):
...
def test_github_14_multi(tmp_path):
    ...
    with py7zr.SevenZipFile(str(testdata_path.joinpath("github_14_multi.7z")), "r") as archive:
...
def test_multiblock(tmp_path):
    with open(testdata_path.joinpath("mblock_1.7z"), mode="rb") as fp:
        archive = py7zr.SevenZipFile(fp)
        archive.extractall(path=tmp_path)
        m = hashlib.sha256()
...
def test_copy(tmp_path):
    ...
    with open(testdata_path.joinpath("copy.7z"), mode="rb") as fp:
        check_archive(
            py7zr.SevenZipFile(fp),
            tmp_path,
            False,
        )
...
def test_copy_2(tmp_path):
    ...
    with open(testdata_path.joinpath("copy_2.7z"), mode="rb") as fp:
        with py7zr.SevenZipFile(fp) as ar:
...
def test_no_main_streams(tmp_path):
    with open(testdata_path.joinpath("test_folder.7z"), mode="rb") as fp:
         archive = py7zr.SevenZipFile(fp)
         archive.extractall(path=tmp_path)
         archive.close()
...
def test_extract_symlink_with_relative_target_path(tmp_path):
    with open(testdata_path.joinpath("symlink.7z"), mode="rb") as fp:
        ...
        os.makedirs(str(tmp_path.joinpath("target")))
        archive.extractall(path="target")
...
def test_extract_emptystream_mix(tmp_path):
    with py7zr.SevenZipFile(str(testdata_path.joinpath("test_6.7z")), "r") as archive:
        archive.extractall(path=tmp_path)


@pytest.mark.files
def test_extract_longpath_file(tmp_path):
    with open(testdata_path.joinpath("longpath.7z"), "rb") as fp:
        with py7zr.SevenZipFile(fp) as archive:
             archive.extractall(path=tmp_path)
...
def test_py7zr_extract_corrupted(tmp_path):
    with pytest.raises(CrcError):
        with py7zr.SevenZipFile(str(testdata_path.joinpath("crc_corrupted.7z")), "r") as archive:
            archive.extract(path=tmp_path)


@pytest.mark.files
def test_extract_lzma2delta(tmp_path):
    with open(testdata_path.joinpath("lzma2delta_1.7z"), "rb") as fp:
        with py7zr.SevenZipFile(fp) as archive:
            archive.extractall(path=tmp_path)
...
def test_extract_lzma_bcj_x86(tmp_path):
    with open(testdata_path.joinpath("lzma_bcj_x86.7z"), mode="rb") as fp:
        with py7zr.SevenZipFile(fp) as ar:
             _dict = ar.readall()
...
def test_extract_lzma_bcj_arm(tmp_path):
    with open(testdata_path.joinpath("lzma_bcj_arm.7z"), mode="rb") as fp:
        with py7zr.SevenZipFile(fp) as ar:
            ar.extractall(tmp_path)
...
def test_extract_lzma_bcj_armt(tmp_path):
    with open(testdata_path.joinpath("lzma_bcj_armt.7z"), mode="rb") as fp:
        with py7zr.SevenZipFile(fp) as ar:
            ar.extractall(tmp_path)
...
def test_extract_lzma_bcj_ppc(tmp_path):
    with open(testdata_path.joinpath("lzma_bcj_ppc.7z"), mode="rb") as fp:
        with py7zr.SevenZipFile(fp) as ar:
            ar.extractall(tmp_path)
...
def test_extract_lzma_bcj_sparc(tmp_path):
    with open(testdata_path.joinpath("lzma_bcj_sparc.7z"), mode="rb") as fp:
        with py7zr.SevenZipFile(fp) as ar:
            ar.extractall(tmp_path)
...
def test_extract_lzma_bcj_2(tmp_path):
    with open(testdata_path.joinpath("lzma_bcj_2.7z"), mode="rb") as fp:
        with py7zr.SevenZipFile(fp) as ar:
            ar.extractall(tmp_path)
...
def test_extract_hidden_linux_folder(tmp_path):
    ...
    with open(testdata_path.joinpath("hidden_linux_folder.7z"), mode="rb") as archive_fp:
        with py7zr.SevenZipFile(archive_fp) as archive:
            assert sorted(archive.getnames()) == [
                hidden_folder_name,
            ]
            archive.extractall(path=tmp_path)
...
def test_extract_hidden_linux_file(tmp_path):
    ...
    with open(testdata_path.joinpath("hidden_linux_file.7z"), mode="rb") as archive_fp:
        with py7zr.SevenZipFile(archive_fp) as archive:
            assert sorted(archive.getnames()) == [
                hidden_file_name,
            ]
